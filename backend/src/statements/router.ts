...
const STMT_COLS =
  'id, client_id, arquivo_nome, storage_path, formato, banco_id, conta_ofx, period_start, period_end, banco_conta_contabil, hist_code_entrada, hist_code_saida, lote_numero, saldo_inicial, saldo_final, complemento_modo, status, erro_msg, totais, created_at, updated_at';
const TXN_COLS =
  'id, ordem, data, descricao_raw, valor, direction, conta_contabil, hist_code, hist_complemento, cod_complemento_hist, ignorado, regra_id, origem_preenchimento';
...
statementsRouter.post('/', upload.single('file'), async (req, res, next) => {
  ...
  try {
    ...
    // Conciliação encadeada: o saldo inicial deste extrato é o saldo final do
    // extrato anterior do cliente. Só cai no valor do cadastro no primeiro.
    let saldoInicial = dto.saldo_inicial;
    if (saldoInicial == null) {
      ...
      saldoInicial = prev ? Number(prev.saldo_final) : Number(client.saldo_inicial ?? 0);
    }
...
statementsRouter.post('/:id/export', async (req, res, next) => {
  try {
    ...
    const { data: txns, error: tErr } = await supabase
      .from('transactions')
      .select('ordem, data, direction, valor, conta_contabil, hist_code, descricao_raw, hist_complemento, ignorado')
      .eq('statement_id', statementId)
      .order('ordem');
    if (tErr) throw mapPgrstError(tErr, 'buscar lançamentos');
